    ...
    <ul>
      {props.todos.map((item, i) => (
        ...
        <div className="w-[100%] md:flex md:flex-column">
          ...
          <div className="w-[100%] md:mr-3 dark:bg-zinc-600 rounded dark:text-white block my-3 px-3 py-3"><span className={item.done ? "line-through" : ""} data-testid={`todo${i}`} onChange={() => props.handleEdit(this)} key={item.text} >{item.text}</span></div>
