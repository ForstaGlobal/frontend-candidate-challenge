  ...
  return (
    <ul>
      {props.todos.map((item, i) => (
        ...
        <div className="w-[100%] md:flex md:flex-column" id={`todo${i}`}>

        <div className="overflow-auto sm:inline-block md:p-3 md:m-3 m-auto md:min-w-[82px] md:w-[82px] w-[100%]">
          <label className="inline-flex relative items-center cursor-pointer">
            <input type="checkbox" onClick={() => props.handleDone(item)} id={`check-${i}`} defaultChecked={item.done} className="sr-only peer"/>
